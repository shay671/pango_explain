...
def test_generate_alias_ancestry_report_contains_known_examples(alias_map, tmp_path):
    ...
    recombinant_rows = generate_recombinant_report(alias_map)
    ...
    def recombinant_for(alias: str) -> str:
        for key, variants in recombinant_rows:
            if key == alias:
                return variants
        raise AssertionError(f"No recombinant entry found for {alias}")
    ...
    assert recombinant_for("XEG") == "JN.3.2.1 , GJ.1.2 , JN.3.2.1 , GJ.1.2"

    report_path = tmp_path / "report.xlsx"
    write_alias_ancestry_workbook(rows, report_path, recombinant_rows)

    with ZipFile(report_path) as archive:
        sheet_xml = archive.read("xl/worksheets/sheet1.xml")
        recombinant_xml = archive.read("xl/worksheets/sheet2.xml")
    ...
    recombinant_root = ET.fromstring(recombinant_xml)
    recombinant_data = recombinant_root.find("ws:sheetData", ns)
    assert recombinant_data is not None

    recombinant_rows_xml = recombinant_data.findall("ws:row", ns)
    assert recombinant_rows_xml

    recombinant_header = row_values(recombinant_rows_xml[0])
    assert recombinant_header == ["Recombinant", "Comprising variants"]

    def recombinant_excel_row(alias: str) -> list[str]:
        for row in recombinant_rows_xml[1:]:
            values = row_values(row)
            if values and values[0] == alias:
                return values
        raise AssertionError(f"No recombinant Excel row found for {alias}")

    assert recombinant_excel_row("XEG")[1] == recombinant_for("XEG")
